...
class BlockAgent(AbstractAgent):
    ...
    def calculate_score(self, phi, timestep, w_t, cluster):
        '''
        Scores a modified long vector phi using w_t * phi, which encodes our projection of how much reward the vector
        will produce, and the ucb, which encodes our confidence that we will gain that reward and the potential of
    ...
    def choose(self, user, contexts, timestep):
        ...
        cluster = self.idx_to_cluster[user]
        cluster_info = self.cluster_info[cluster]
        user = cluster_info.user_to_user_in_cluster[user]
        ...
        for context in contexts:
            ...
            for i in range(self.vector_size):
                ...
                # examined. The block begins at user * self.vector_size, following which the vector is placed, with
                # a length vector_size
                new_context[i + user * self.vector_size] = np.float32(context_vector[i])
    ...
    def update(self, user, context, reward):
        ...
        cluster_info.bias = cluster_info.bias + phi * reward
